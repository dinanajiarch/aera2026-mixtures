...
move_files(files, here("lpa_enumeration"), overwrite = TRUE)
```
...
select_models <- enum_fit(output_enum) %>%
...
Using `MplusAutomation` code (NOTE: The `plotMixtures()` function is used for plotting LPA models only (i.e., means & variances))
